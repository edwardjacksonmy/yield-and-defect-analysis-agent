...
- **ML-based defect pattern classification** — predicts WM-811K defect patterns (Center, Donut, Edge-Ring, etc.) from spatial fail-die distributions using a trained Random Forest; no pre-labeled `defect_code` required
...
│Tools   │       │Visual     │     │ Tools      │
│        │       │Tools      │     │            │
│• Yield │       │• ML Pattern│    │• Root Cause│
│• Defect│       │  Classify │     │• Report Gen│
│• Ingest│       │• DBSCAN   │     │• Historical│
│        │       │• Wafer Map│     │            │
└───┬────┘       └─────┬─────┘     └─────┬──────┘
    │                  │                  │
┌───▼──────────────────▼──────────────────▼──────────┐
...
│
├── agent/
│   ├── core.py                  # LangChain ReAct agent setup (9 tools)
│   ├── db_chat.py               # Chat history persistence (PostgreSQL)
│   └── tools/
│       ├── data_ingestion.py    # CSV parsing & validation (defect_code optional)
│       ├── pattern_classifier.py# ML defect pattern prediction (Random Forest)
...
│
├── data/
│   ├── train_classifier.py      # Train Random Forest on WM-811K spatial features
│   ├── preprocessing.py         # WM-811K pickle → DataFrame converter
│   ├── db_seeder.py             # Historical data loader for PostgreSQL
│   ├── wafer_pattern_model.joblib  # Trained classifier (generated by train_classifier.py)
...
| # | Tool | Description |
|---|---|---|
| 1 | `data_ingestion_tool` | Parses and validates uploaded CSV; stores to session state and DB. `defect_code` column is optional. |
| 2 | `pattern_classifier_tool` | Predicts WM-811K defect pattern per wafer using a trained Random Forest. Derives 16 spatial features (zone fail rates, angular entropy, PCA linearity, DBSCAN clusters) from `die_x`, `die_y`, `pass_fail` — no pre-labeled defect code needed. |
| 3 | `yield_calculator_tool` | Computes overall and per-wafer yield rates; flags critical batches (< 80%) |
| 4 | `defect_analyzer_tool` | Ranks defect types using WM-811K taxonomy; surfaces process-specific insights |
| 5 | `spatial_clustering_tool` | Runs DBSCAN on failed die coordinates to distinguish systematic vs. random defects |
| 6 | `wafer_visualizer_tool` | Generates Plotly wafer maps, defect bar charts, and yield heatmaps |
| 7 | `historical_query_tool` | Queries PostgreSQL for yield trends and lot comparisons |
| 8 | `root_cause_tool` | Returns ranked hypotheses and recommended actions per defect pattern |
| 9 | `report_generator_tool` | Compiles all analysis sections into a downloadable markdown report |
...
### 6. (Optional) Train the Defect Pattern Classifier

This trains a Random Forest on WM-811K wafer maps to predict defect patterns from spatial die data. Required if your CSV files do not include a `defect_code` column.

```bash
python data/train_classifier.py data/LSWMD.pkl
# Saves: data/wafer_pattern_model.joblib
```

Options:
```bash
python data/train_classifier.py data/LSWMD.pkl --max-wafers 5000   # faster for testing
python data/train_classifier.py data/LSWMD.pkl --output data/my_model.joblib
```

### 7. (Optional) Seed Historical Data
...
|---|---|---|---|
| `lot_id` | string | Yes | Lot identifier |
| `wafer_id` | string/int | Yes | Wafer identifier within lot |
| `die_x` | int | Yes | Die X coordinate |
| `die_y` | int | Yes | Die Y coordinate |
| `pass_fail` | int | Yes | `1` = pass, `0` = fail |
| `defect_code` | string | No | One of the 9 WM-811K pattern labels. If omitted, the agent automatically calls `pattern_classifier_tool` to predict the pattern from spatial die data. |
...
| **ML Classifier** | scikit-learn Random Forest, joblib |
| **Frontend** | Streamlit |
